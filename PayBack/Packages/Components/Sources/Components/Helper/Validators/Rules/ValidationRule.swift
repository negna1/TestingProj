...
public class ValidationRule {
     ...
     */
    public func validateField() -> ValidateError? {
        return rules.filter{
            return !$0.validate(field.validationText)
            }.map{ rule -> ValidateError in return ValidateError(field: self.field, errorLabel:self.errorLabel, error: rule.errorMessage()) }.first
    }
}
